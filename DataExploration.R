...
# initial idea came from: https://www.kaggle.com/users/993/ben-hamner/bike-sharing-demand/bike-rentals-by-time-and-temperature
gg1 <- ggplot(train, aes(timeofday, count, color=9/5*atemp+32)) +
...
## FFT of time of day, work vs. nonwork days
train.work <- filter(train, workingday == "Workday")
train.nonwork <- filter(train, workingday == "Holiday/Weekend")

train.work <- mutate(train.work, 
                     freq=1:nrow(train.work), 
                     count.fft=fft(count))
train.nonwork <- mutate(train.nonwork, 
                        freq=1:nrow(train.nonwork), 
                        count.fft=fft(count))

train.fft <- rbind(train.work, train.nonwork)

gg5 <- ggplot(train.fft, aes(freq, Mod(count.fft)/max(Mod(count.fft)))) +
    facet_grid(. ~ workingday, scales="free") +
    geom_line(color="dodgerblue") +
    theme(plot.title = element_text(size = rel(2))) +
    labs(title="Bike Rental Demand \nFrequency Magnitude Plot")+
    labs(x="Frequency (Units??)")
windows()
print(gg5)
